...
def main():
    ...
    dsize = 10
    target_domain = np.fromfunction(
        lambda x, y: (
            (x - dsize / 2) ** 2 + (y - dsize / 2) ** 2 <= (dsize / 4) ** 2
        ).astype(int),
        (dsize, dsize),
    )
    start_domain = np.random.randint(
        3, size=(dsize, dsize)
    )  # Random initial grid with 3 classes
    ...
    for _ in range(num_train_iter):
        ...
        domain_next = start_domain
        for _ in range(10):
            domain_next = update_grid(domain_next, model)

        domain_tensor = torch.tensor(
            start_domain, dtype=torch.float32, requires_grad=True
        )
        domain_next_tensor = torch.tensor(
            domain_next, dtype=torch.float32, requires_grad=True
        )

        # measure of how close generated image is to the original
        similarity = torch.mean(
            torch.abs(
                domain_next_tensor - torch.tensor(target_domain, dtype=torch.float32)
            )
        )
        loss = 1 - similarity
    ...
    _, axs = plt.subplots(1, num_iterations + 2, figsize=(15, 5))
