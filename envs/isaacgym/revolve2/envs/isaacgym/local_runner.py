...
class LocalRunner(Runner):
    class Simulator:
        ...
        def _create_envs(self) -> List[GymEnv]:
            ...
            for i, val in enumerate(self._batch.environments):
                ...
                botfile.writelines(
                    physbot_to_urdf(val.robot, f"robot_{i}", Vector3(), Quaternion())
                )
                botfile.close()
                asset_root = os.path.dirname(botfile.name)
                urdf_file = os.path.basename(botfile.name)
                robot_asset = self._gym.load_urdf(self._sim, asset_root, urdf_file)
                ...
                robot_handle = self._gym.create_actor(
                    env, robot_asset, pose, f"robot_{i}", 0, 0
                )

                gymenvs.append(self.GymEnv(env, [robot_handle]))
        ...
        def _get_state(self) -> State:
            ...
            for gymenv in self._gymenvs:
                ...
                for actor in gymenv.actors:
                    pose = self._gym.get_actor_rigid_body_states(
                        gymenv.env, actor, gymapi.STATE_POS
                    )["pose"]
