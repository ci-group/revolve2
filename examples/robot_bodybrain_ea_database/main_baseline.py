...
def get_fitnesses(genotypes: list[Genotype], evaluator: Evaluator) -> list[float]:
    amt_ind = len(genotypes)
    tmp_developed: list[ModularRobot] = [None] * amt_ind
    tmp_brains: list[BrainCpgNetworkNeighborRandom] = [None] * amt_ind
    structs: list[CpgNetworkStructure] = [None] * amt_ind
    des: list[DifferentialEvolution] = [None] * amt_ind

    i = 0
    for genotype in genotypes:
        tmp = genotype.develop()
        struct, _ = active_hinges_to_cpg_network_structure_neighbor(tmp.body.find_modules_of_type(ActiveHinge))

        tmp_developed[i] = tmp
        tmp_brains[i] = tmp.brain
        structs[i] = struct
    ...
    for gen in range(N_GEN):
        robots = []
        for tmp, de, struct in zip(tmp_developed, des, structs):
            robots.extend([get_robot_from_x(tmp.body, genome, struct) for genome in de.x_current])

        all_f = np.array_split(-np.array(evaluator.evaluate(robots)), amt_ind)
        j = 0
        for de in des:
            de.f = all_f[j]
            de.new_pop()
...
def main() -> None:
    ...
    dbengine = open_database_sqlite(
        f"baseline_{config.DATABASE_FILE}", open_method=OpenMethod.NOT_EXISTS_AND_CREATE
    )
