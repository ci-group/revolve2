...
class BrainGraph:
    def __init__(self, brain, name="brain", typename="brain"):
        ...
        self.graph = Digraph(
            typename, filename=name, format=format, node_attr={"margin": "0"}
        )
        self.brain = brain
            ...
            if node not in duplicates:
                ...
                if node_id in params:
                    if params[node_id].period is not None:
                        period = (
                            round(float(params[node_id].period), decimals)
                            if round_params
                            else float(params[node_id].period)
                        )
                        text += "\n period: {0}".format(period)
                    if params[node_id].phase_offset is not None:
                        phase_offset = (
                            round(float(params[node_id].phase_offset), decimals)
                            if round_params
                            else float(params[node_id].phase_offset)
                        )
                        text += "\n phase_offset: {0}".format(phase_offset)
                    if params[node_id].amplitude is not None:
                        amplitude = (
                            round(float(params[node_id].amplitude), decimals)
                            if round_params
                            else float(params[node_id].amplitude)
                        )
                        text += "\n amplitude: {0}".format(amplitude)
                    if params[node_id].bias is not None:
                        bias = (
                            round(float(params[node_id].bias), decimals)
                            if round_params
                            else float(params[node_id].bias)
                        )
                        text += "\n bias: {0}".format(bias)
                    if params[node_id].gain is not None:
                        gain = (
                            round(float(params[node_id].gain), decimals)
                            if round_params
                            else float(params[node_id].gain)
                        )
                        text += "\n gain: {0}".format(gain)
        ...
        for connection in self.brain.connections:
            if round_params:
                self.add_edge(
                    connection.src,
                    connection.dst,
                    round(float(connection.weight), decimals),
                )
            else:
