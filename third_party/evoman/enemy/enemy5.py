        ...
        if game.start == 1:
            ...
            if (atack2 == 1 and not self.gun_cooldown) :
                ...
                for i in range(0,aux):
                    self.twists.append(Bullet_e5((self.rect.x + (self.direction*(i*30)) ,self.rect.top + (self.direction*(i*20))  ), self.direction, game.player.rect, len(self.twists), game.sprite_enemy))

                self.timeenemy = 0 # reinicializes enemy timer

            if game.player.atacked == 1:

                self.twists.append(Bullet_e5((self.rect.x ,self.rect.top ), self.direction, game.player.rect, len(self.twists), game.sprite_enemy))
